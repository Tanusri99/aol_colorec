...
class VideoFrame:
    def __init__(self, buffer: Gst.Buffer, caps: Gst.Caps, roi: hailo.HailoROI):
        self._buffer = buffer
        self._roi = roi
        # Initialize GstVideo.VideoInfo using the correct method
    ...
    @classmethod
    def numpy_array_from_buffer(cls, map_info: Gst.MapInfo, video_info: GstVideo.VideoInfo):
        numpy_frame = np.ndarray(shape=(video_info.height, video_info.width, 3),
                                 dtype=np.uint8,
                                 buffer=map_info.data)
